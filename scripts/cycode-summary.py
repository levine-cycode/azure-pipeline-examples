...
def _md_body_for_type(r: dict, scan_type: str) -> list[str]:
    ...
    def _file_line() -> str:
        ...
        we have an ADO file URL, otherwise as plain `path:line` code."""
        file = r.get("file") or ""
        line = r.get("line") or ""
        url = r.get("ado_file_url") or ""
        if not file:
            return ""
...
def _row_for_type(r: dict, scan_type: str) -> str:
    ...
    file_link = r.get("ado_file_url") or ""
    file_inner = (
...
def main() -> int:
    ...
    # Populate ADO file URL if environment variables are available
    ...
    if collection_uri and project and repo and ref:
        for r in rows:
            r["ado_file_url"] = azure_file_url(r, collection_uri, project, repo, ref)
    ...
    if args.html:
        # Pull build context from the same ADO env vars the Markdown summary uses
        # so the HTML header shows Repo / Branch / Commit / Scan path inline.
